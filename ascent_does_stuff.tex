...
\fix{Hank's attempt at the intro paragraph}

This section describe Ascent's primary abstractions and surveys its general capabilities.
%
Ascent has organized its capabilities into five types of in situ tasks, which
...
%
Figure~\ref{fig:ascent_example} shows such an example.
...
and that data is sent into a pipeline, which applies threshold and clip operations.
%
The result of the pipeline is used in multiple ways, including a scene (which renders
multiple images) and two extracts for two different I/O libraries.}
\end{figure}
